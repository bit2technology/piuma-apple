// Thanks to Ole Begemann https://oleb.net/2018/uialertcontroller-textfield/
...
extension UIAlertController {
    ...
    public convenience init(title: String, message: String? = nil, cancelButtonTitle: String, okButtonTitle: String, validate validationRule: TextValidationRule = .noRestriction, textFieldConfiguration: ((UITextField) -> Void)? = nil, onCompletion: @escaping (TextInputResult) -> Void) {
        ...
        let okAction = UIAlertAction(title: okButtonTitle, style: .default) { [unowned self] _ in
        ...
        addTextField {
            ...
            textFieldObserver = TextFieldObserver(textField: $0, valueChanged: { okAction.isEnabled = validationRule.isValid($0.text ?? "") }, shouldReturn: { validationRule.isValid($0.text ?? "") })
        }
